			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.dianpumingcheng" type="text" placeholder="店铺名称" ></input>
					</view>
				<view class="action">
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0px 0px 64rpx 0px","background":"none","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"0px","margin":"0 0 40rpx 0","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
				  ...
				  <view class="listmr" :style='{"minHeight":"192rpx","padding":"12rpx 0","boxShadow":"6rpx 6rpx 6rpx rgba(180,180,180,.3),inset 0px 0px 112rpx 0px #d7f6f2","margin":"12rpx 0px 12rpx 0","borderRadius":"16rpx","background":"#fff","flex":"1","width":"100%","height":"auto"}'>
					<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx","margin":"0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","width":"100%","fontSize":"28rpx","lineHeight":"48rpx","textOverflow":"ellipsis","height":"48rpx"}'>{{product.dianpumingcheng}}</view>
				  </view>
					...
					<view :style='{"padding":"0px","margin":"20rpx 0 0","textAlign":"right","display":"flex","width":"100%","float":"right","justifyContent":"flex-end","height":"auto"}'>
						<view :style='{"cursor":"pointer","margin":"0px 16rpx 0 0","display":"flex"}' v-if="(userid && isAuth('meishidianpu','修改')) || (!userid && isAuthFront('meishidianpu','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"cursor":"pointer","margin":"0px 16rpx 0 0","display":"flex"}' v-if="(userid && isAuth('meishidianpu','删除')) || (!userid && isAuthFront('meishidianpu','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.dianpumingcheng="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.dianpumingcheng){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`meishidianpu`, params);
                } else {
                    res = await this.$api.list(`meishidianpu`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('meishidianpu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.dianpumingcheng){
					searchForm['dianpumingcheng'] = '%' + this.searchForm.dianpumingcheng + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`meishidianpu`, searchForm);
                } else {
                    res = await this.$api.list(`meishidianpu`, searchForm);
                }
				// 如果是第一页数据置空
